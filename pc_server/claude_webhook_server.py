...
if __name__ == "__main__":
    ...
    is_local_only_host = HOST in {"127.0.0.1", "localhost"}
    if not TOKEN and not ALLOW_INSECURE_NO_TOKEN and not is_local_only_host:
        raise SystemExit(
            "Refusing to start without CLAUDE_VOICE_TOKEN on non-localhost bind addresses. Set CLAUDE_VOICE_TOKEN, bind to 127.0.0.1/localhost, or override with CLAUDE_VOICE_ALLOW_INSECURE_NO_TOKEN=1 for local development only."
        )
    ...
    log(f"Bearer auth: {'enabled' if TOKEN else 'DISABLED'}")
